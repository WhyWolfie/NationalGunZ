...
void MMatchServer::OnRequestDeleteChar(const MUID& uidPlayer, const int nCharIndex, const char* szCharName)
{
	...
	if (strstr(szCharName, "'")) {
		// SQL injection
		// That's it ok and look what i did 
		return;
	}
...
bool MMatchServer::CharFinalize(const MUID& uidPlayer)
{
	...
	{
		pUpdateAccountLastLoginTimeJob->Input( pObj->GetAccountInfo()->m_nAID, pObj->m_dwLastVIPAnnounce, pObj->m_nVIPWalls );
