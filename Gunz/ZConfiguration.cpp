...
bool ZConfiguration::LoadConfig(const char* szFileName)
{
	...
	if (!parentElement.IsEmpty())
	{
		...
		if( parentElement.FindChildNode(ZTOK_MACRO, &childElement) )
		{
			...
			childElement.GetChildContents(m_Macro.szMacro[8], ZTOK_MACRO_F9, 255);
		}
		//EXTRA
		if (parentElement.FindChildNode(ZTOK_EXTRA, &childElement))
		{
			childElement.GetChildContents(&m_Extra.bDamageCounter, "DAMAGEC");
			childElement.GetChildContents(&m_Extra.bDisableSpecial, ZTOK_EXTRA_DISABLESPECIAL);
			childElement.GetChildContents(&m_Extra.bnomassive, ZTOK_EXTRA_NOMASSIVE);
			childElement.GetChildContents(&m_Extra.bfps, ZTOK_EXTRA_FPS);
			childElement.GetChildContents(&m_Extra.bhidecrosshair, ZTOK_EXTRA_HIDECROSSHAIR);
			childElement.GetChildContents(&m_Extra.bbullet, ZTOK_EXTRA_BULLET);
			childElement.GetChildContents(&m_Extra.bsword, ZTOK_EXTRA_SWORD);
			childElement.GetChildContents(&m_Extra.bhpap, ZTOK_EXTRA_HPAP);
			childElement.GetChildContents(&m_Extra.bvisible, ZTOK_EXTRA_VISIBLE);
			childElement.GetChildContents(&m_Extra.bwheellock, ZTOK_EXTRA_WHEELLOCK);
			childElement.GetChildContents(&m_Extra.bnonumbers, ZTOK_EXTRA_NONUMBERS);
			childElement.GetChildContents(&m_Extra.bscops, ZTOK_EXTRA_SCOPS);
			childElement.GetChildContents(&m_Extra.bnosmoke, ZTOK_EXTRA_NOSMOKE);
			childElement.GetChildContents(&m_Extra.bdamage, ZTOK_EXTRA_DAMAGE);
			childElement.GetChildContents(&m_Extra.btime, ZTOK_EXTRA_TIME);
			childElement.GetChildContents(&m_Extra.bposition, ZTOK_EXTRA_POSITION);
			childElement.GetChildContents(&m_Extra.bautorec, ZTOK_EXTRA_AUTOREC);
		...
		if (parentElement.FindChildNode(ZTOK_ETC, &childElement))
		{
			...
			childElement.GetChildContents(&m_Etc.nTime, "UNIX");
...
//	if( parentElement.FindChildNode(ZTOK_MACRO, &childElement) )
	...
	// Etc
	{
		...
		// FrameLimit
		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement("UNIX");
		sprintf(temp, "%i", m_Etc.nTime);
		aElement.SetContents(temp);
////////
			{
		...
		MXmlElement		aElement;
		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_EXTRA_DISABLESPECIAL);
		sprintf(temp, "%s", m_Extra.bDisableSpecial?"TRUE":"FALSE");
		aElement.SetContents(temp);


		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_EXTRA_NOMASSIVE);
		sprintf(temp, "%s", m_Extra.bnomassive?"TRUE":"FALSE");
		aElement.SetContents(temp);


		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement("DAMAGEC");
		sprintf(temp, "%s", m_Extra.bDamageCounter?"TRUE":"FALSE");
		aElement.SetContents(temp);


		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_EXTRA_FPS);
		sprintf(temp, "%s", m_Extra.bfps?"TRUE":"FALSE");
		aElement.SetContents(temp);


		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_EXTRA_HIDECROSSHAIR);
		sprintf(temp, "%s", m_Extra.bhidecrosshair?"TRUE":"FALSE");
		aElement.SetContents(temp);


		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_EXTRA_BULLET);
		sprintf(temp, "%s", m_Extra.bbullet?"TRUE":"FALSE");
		aElement.SetContents(temp);


		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_EXTRA_SWORD);
		sprintf(temp, "%s", m_Extra.bsword?"TRUE":"FALSE");
		aElement.SetContents(temp);


		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_EXTRA_HPAP);
		sprintf(temp, "%s", m_Extra.bhpap?"TRUE":"FALSE");
		aElement.SetContents(temp);


		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_EXTRA_VISIBLE);
		sprintf(temp, "%s", m_Extra.bvisible?"TRUE":"FALSE");
		aElement.SetContents(temp);


		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_EXTRA_WHEELLOCK);
		sprintf(temp, "%s", m_Extra.bwheellock?"TRUE":"FALSE");
		aElement.SetContents(temp);


		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_EXTRA_NONUMBERS);
		sprintf(temp, "%s", m_Extra.bnonumbers?"TRUE":"FALSE");
		aElement.SetContents(temp);


		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_EXTRA_SCOPS);
		sprintf(temp, "%s", m_Extra.bscops?"TRUE":"FALSE");
		aElement.SetContents(temp);


		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_EXTRA_NOSMOKE);
		sprintf(temp, "%s", m_Extra.bnosmoke?"TRUE":"FALSE");
		aElement.SetContents(temp);

		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_EXTRA_DAMAGE);
		sprintf(temp, "%s", m_Extra.bdamage?"TRUE":"FALSE");
		aElement.SetContents(temp);

		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_EXTRA_TIME);
		sprintf(temp, "%s", m_Extra.btime?"TRUE":"FALSE");
		aElement.SetContents(temp);

		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_EXTRA_POSITION);
		sprintf(temp, "%s", m_Extra.bposition?"TRUE":"FALSE");
		aElement.SetContents(temp);

	    parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_EXTRA_AUTOREC);
		sprintf(temp, "%s", m_Extra.bautorec?"TRUE":"FALSE");
		aElement.SetContents(temp);
...
void ZConfiguration::Init()
{
	...
	//Server
	strcpy(m_szServerIP, "80.179.155.231");
	m_nServerPort = 6000;
