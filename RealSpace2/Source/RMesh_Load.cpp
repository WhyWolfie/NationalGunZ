...
//			Node.GetAttribute(szSize,"size");//³ªÁß¿¡ ÇÊ¿äÇÏ¸é »ì¸®ÀÚ.. Áö±ÝÀº ±×¶§±×¶§ ¹Ù²Ù´Â°ÍÀÌ ´õ ±ÍÂú´Ù..
			m_ani_mgr.MakeListMap( (int)eq_weapon_end );//ÃÖ´ë¸ð¼ÇÅ¸ÀÙ¸¸Å­ ¸¸µé¾îÁØ´Ù..
		}
			...
			if(nGameMotion==1) { // °ÔÀÓ ¸ð¼ÇÀº ³ªÁß¿¡ ·Îµù
			...
			if(pAni) {
				...
				if(SoundFileName[0]==NULL) {//»ç¿îµå°¡ µî·ÏµÇÁö ¾Ê¾ÒÀ¸¸é ¿¡´Ï¸ÞÀÌ¼Ç ÆÄÀÏ ÀÌ¸§ÀÌ ±âº» »ç¿îµåÀÌ¸§..
					int len = (int) strlen(FileName);
					strncpy(SoundFileName,FileName,len-8);//.elu.ani »ý·«...
...
bool RMesh::ReadXml(char* filename)
{
	...
	string BackupName = filename;//ÀÔ·Â ¹ÞÀº°ÍÀÌ ³»ºÎ¿¡¼­ ¹Ù²ï´Ù.
...
// ·Îµù ¼Óµµ¸¦ ¿Ã¸®·Á¸é ¸Æ½ºÀÍ½ºÆ÷ÅÍ¿¡¼­ ¹Ì¸®ÇØÁÖ¾îµµ µÇÁö¸¸ º¯°æ »çÇ×ÀÌ ÀÖÀ»°æ¿ì
// ÀÍ½ºÆ÷Æ®¸¦ ´Ù½ÃÇØ¾ßÇÔ...
...
//		pMeshNode->ConnectToNameID();//ÀÌ¸§À» ID ·Î ¹Ù²Û´Ù..
	}
	...
	else if( NCMPNAME("eq_wr",5) || NCMPNAME("eq_wl",5) || NCMPNAME("eq_wd",5) ) {// ÀüºÎ ¼Ò¹®ÀÚÈ­~
		// ¿À¸¥¼Õ ¹«±â ´õ¹Ì Á¾·ù
		...
		if(NCMPNAME("eq_head",7) )	{
			...
			pMeshNode->m_AlphaSortValue = 1.f;//¾ËÆÄ°ªÀÌ ÀÖ´Â°æ¿ì ¼ÒÆÃ¿ì¼±¼øÀ§
		}
...
//			pMeshNode->m_isCollisionMesh = true;
	...
	else if(NCMPNAME("collision_",10)) {	// Ãæµ¹ÀÌ³ª picking ¿ë ¸ðµ¨
		pMeshNode->m_isCollisionMesh = true;
		pMeshNode->m_isDummyMesh = true;
	}
/*
...
//	"muzzle_flash"		//ÃÑ±¸È­¿°
//	"empty_cartridge01"	//ÀÏ¹ÝÅºÇÇ
//	"empty_cartridge02"	//»êÅºÅºÇÇ
...
// ´õ¹Ì¿ë ¸ðµ¨¿¬°á.. ( ¾È°æÀÌ³ª.. °¡¹æµî )
// °¢°¢ÀÇ ºÎ¸ð´Â ¿¡´Ï¸ÞÀÌ¼ÇÀÌ ¿¬°áµÉ¶§¸¶´Ù base matrix °¡ Àç¼³Á¤ µÈ´Ù.
// ¹Ì¸® local matrix ¸¦ °¡Áö°í ÀÖ´Ù°¡ ºÙ¿©ÁÖ´Â ¿ªÇÒ¸¸ÇÏÀÚ..
// ¸Þ½Ã°¡ read µÉ¶§¸¸ ¾²ÀÌµµ·Ï µÇ¾îÀÖ´Ù..¾Æ¹«°÷¿¡¼­³ª »ç¿ëÇÏ·Á¸é ºÎ¸ð¿¬°áµî¿¡ ÁÖÀÇÇØ¼­ ´Ù½ÃÀÛ¼º...
...
bool RMesh::AddNode(char* name,char* pname,rmatrix& base_mat)
{
	...
	pMeshNode->m_mat_base	= base_mat;	// À§Ä¡ Àâ¾ÆÁÖ°í.. Ãâ·ÂµÈ°ªÀ¸·Î..
	pMeshNode->m_mat_ref	= base_mat;
	pMeshNode->m_mat_result = base_mat;
	pMeshNode->m_mat_add	= base_mat; // ·ÎÄÃÀ» À¯ÁöÇÏ¸é¼­ ³ªÁß¿¡ »ç¿ë..
...
//	CalcLocalMatrix(pMeshNode);//»«´Ù..ºÎ¸ð ¿¬°á µîÀÌÁö¸¸ ÀÌÈÄ¿¡ È£Ãâ.. local matrix µîÀ» °è»ê..

	return true;
}


// Á¦°Å½Ã ÀÚ½ÅÀ» ºÎ¸ð·Î °¡Áö´Â ¿ÀºêÁ§Æ®¿¡ ´ëÇØ¼­°í·Á~
...
//	if( RIsSupportVS() )
	{
		for( int i = 0 ; i < m_data_num; ++i )
		{
			...
			if( pMeshNode->m_physique && pMeshNode->m_point_color_num == 0 && isShader) // cloth ¸öÅëÀÌ ¾Æ´Ñ.. ¸ðµ¨µé.. + ¸ó½ºÅÍµé
			{
				pMeshNode->MakeVSVertexBuffer();
			}

			if(pMeshNode->m_physique) {	// ¸öÅë... color ¹öÅØ½º¸¦ Æ÷ÇÔÇÑ°Íµé..
				// CreateVSVertexBuffer ´Â ÀÎµ¦½º¹öÆÛ´Â »ý¼ºÇÏÁö ¾Ê°í ¿©±â¼­ »ý¼ºÇÑ°É »ç¿ëÇÑ´Ù...
				pMeshNode->MakeNodeBuffer( USE_VERTEX_SW );
			}
			else { // ÀÏ¹Ý ¸ðµ¨µé.. ¹öÅØ½º ¿¡´ÏÀÇ °æ¿ì sw ¸¸ »ç¿ë.. ( ¹öÅØ½º ¿¡´Ï ¿¬°áµÇ±âÀü¿¡´Â ½Äº°ºÒ°¡ )
...
{
	// ÆÄÃ÷·Î µî·ÏµÇ´Â ¸ðµ¨µéÀº base model ¿¡ ´ëÇØ¼­ ¿¬°áÇØ Áà¾ß ÇÑ´Ù...
	// ConnectPhysiqueParent °¡ ±× ¿ªÇÒÀ» ÇÏ°í ÀÖÀ½..
		...
		if(pMeshNode->m_physique_num) {
			...
			for(int q=0;q<pMeshNode->m_physique_num;q++) {
				...
				for(j=0;j<t_p_num;j++) {
					...
					ret = _FindMeshId(&pMeshNode->m_physique[q].m_parent_name[j][0]);//·Îµù½Ã¸¸~
...
/*
			if( pMeshNode->m_isDummyMesh==false )// ½ºÅ²Á¤º¸°¡ ÀÖ´Â »óÅÂ¿¡¼­ ¸ðµ¨ ³ëµå¶ó¸é BaseBoneMatrix ¸¦ ¸¸µé¾îÁØ´Ù.
			{
				// »ç¿ëÇÏ´Â º»ÀÇ °¹¼ö - ¿¡´Ï¸ÞÀÌ¼ÇÀÌ ¹Ù²î´õ¶óµµ ¹Ù²îÁö ¾Ê´Â´Ù.
...
//			mlog("%s physique vertex %d , p1 %d , p2 %d , p3 %d\n",pMeshNode->m_Name,pMeshNode->m_physique_num,p1,p2,p3);
		...
		if( m_isNPCMesh ) // npc ÀÇ ¼Õ¿¡µç ¹«±â ±¸ºÐÇÏ±â..
		{ 
...
//			RMatInv(inv,pPN->m_mat_base);								// ºÎ¸ð ÀÎ¹ö½º µî·Ï
...
/*
			if(pPN->m_Parent[0]==0) {
				...
				memcpy(&pNode->m_mat_parent_inv,&inv,sizeof(D3DXMATRIX));//ºÎ¸ðÀÇ ºÎ¸ð¿¡´ëÇÑ »ó´ë ÀÎ¹ö½º°ª
			}
*/
...
bool RMesh::ConnectPhysiqueParent(RMeshNode* pNode)
{
	...
	for(int i=0;i<pNode->m_physique_num;i++) {
		...
		if(p_num > MAX_PHYSIQUE_KEY)	// ÃÖ´ë 4°³ Áö¿ø
...
//		node->m_power = 80.f;
		...
		if(m_phd_t->ver > EXPORTER_MESH_VER2) {//ver3 ????
			int twoside=0;
		...
		if(m_phd_t->ver > EXPORTER_MESH_VER7 )//ver8 ????
		{
			int alpha_test = 0;
...
//			result_mat2 = scalepivotinv * scalemat * scalepivot;
		...
		if(pMeshNode->m_face_num) {
			...
			else if(m_phd_t->ver > EXPORTER_MESH_VER2) {//ver3 ????
...
//	m_mtrl_list_ex.Restore(RGetDevice(),Path);
	...
	CheckNodeAlphaMtrl();// °¢³?µ? alpha mtrl ??°¡ ?¼?©..
