...
if( REnumAdapterMode( D3DADAPTER_DEFAULT,format[i], idm,  &ddm ))
	...
	{	
		...
		MButton* pDisableSpecial = (MButton*)pResource->FindWidget("DisableSpecial"); //Disable Special Items From Item Rotation
		if( pDisableSpecial )
		{
			pDisableSpecial->SetCheck(ZGetConfiguration()->GetExtra()->bDisableSpecial);
		}

		MButton* pnomassive = (MButton*)pResource->FindWidget("nomassive");
		if( pnomassive )
		{
			pnomassive->SetCheck(ZGetConfiguration()->GetExtra()->bnomassive);
		}

		MButton* pfps = (MButton*)pResource->FindWidget("fps");
		if( pfps )
		{
			pfps->SetCheck(ZGetConfiguration()->GetExtra()->bfps);
		}

		MButton* phidecrosshair = (MButton*)pResource->FindWidget("hidecrosshair");
		if( phidecrosshair )
		{
			phidecrosshair->SetCheck(ZGetConfiguration()->GetExtra()->bhidecrosshair);
		}

		MButton* pbullet = (MButton*)pResource->FindWidget("bullet");
		if( pbullet )
		{
			pbullet->SetCheck(ZGetConfiguration()->GetExtra()->bbullet);
		}

				MButton* psword = (MButton*)pResource->FindWidget("sword");
		if( psword )
		{
			psword->SetCheck(ZGetConfiguration()->GetExtra()->bsword);
		}

		MButton* phpap = (MButton*)pResource->FindWidget("hpap");
		if( phpap )
		{
			phpap->SetCheck(ZGetConfiguration()->GetExtra()->bhpap);
		}

		MButton* pvisible = (MButton*)pResource->FindWidget("visible");
		if( pvisible )
		{
			pvisible->SetCheck(ZGetConfiguration()->GetExtra()->bvisible);
		}

		MButton* pwheellock = (MButton*)pResource->FindWidget("wheellock");
		if( pwheellock )
		{
			pwheellock->SetCheck(ZGetConfiguration()->GetExtra()->bwheellock);
		}

				MButton* pnonumbers = (MButton*)pResource->FindWidget("nonumbers");
		if( pnonumbers )
		{
			pnonumbers->SetCheck(ZGetConfiguration()->GetExtra()->bnonumbers);
		}

		MButton* pscops = (MButton*)pResource->FindWidget("scops");
		if( pscops )
		{
			pscops->SetCheck(ZGetConfiguration()->GetExtra()->bscops);
		}

				MButton* pnosmoke = (MButton*)pResource->FindWidget("nosmoke");
		if( pnosmoke )
		{
			pnosmoke->SetCheck(ZGetConfiguration()->GetExtra()->bnosmoke);
		}


		MButton* pdamage = (MButton*)pResource->FindWidget("damage");
		if( pdamage )
		{
			pdamage->SetCheck(ZGetConfiguration()->GetExtra()->bdamage);
		}

		MButton* ptime = (MButton*)pResource->FindWidget("time");
		if( ptime )
		{
			ptime->SetCheck(ZGetConfiguration()->GetExtra()->btime);
		}

		MButton* position = (MButton*)pResource->FindWidget("position");
		if( position )
		{
			position->SetCheck(ZGetConfiguration()->GetExtra()->bposition);
		}
		MButton* pautorec = (MButton*)pResource->FindWidget("autorec");
		if( pautorec )
		{
			pautorec->SetCheck(ZGetConfiguration()->GetExtra()->bautorec);
		}
...
//		ZGetInput()->UnregisterActionKey(i);
	...
	*/
	...
	{	// Etc
		...
		if(pDisableSpecial)
		{
			Z_EXTRA_DISABLESPECIAL = pDisableSpecial->GetCheck();

				if (Z_EXTRA_DISABLESPECIAL)
					ZGetConfiguration()->GetExtra()->bDisableSpecial = true;
				else
					ZGetConfiguration()->GetExtra()->bDisableSpecial = false;
		}

		MButton* pnomassive = (MButton*)pResource->FindWidget("nomassive");
		if(pnomassive)
		{
			Z_EXTRA_NOMASSIVE = pnomassive->GetCheck();

				if (Z_EXTRA_NOMASSIVE)
					ZGetConfiguration()->GetExtra()->bnomassive = true;
				else
					ZGetConfiguration()->GetExtra()->bnomassive = false;
		}

		MButton* pfps = (MButton*)pResource->FindWidget("fps");
		if(pfps)
		{
			Z_EXTRA_FPS = pfps->GetCheck();

				if (Z_EXTRA_FPS)
					ZGetConfiguration()->GetExtra()->bfps = true;
				else
					ZGetConfiguration()->GetExtra()->bfps = false;
		}

		MButton* phidecrosshair = (MButton*)pResource->FindWidget("hidecrosshair");
		if(phidecrosshair)
		{
			Z_EXTRA_HIDECROSSHAIR = phidecrosshair->GetCheck();

				if (Z_EXTRA_HIDECROSSHAIR)
					ZGetConfiguration()->GetExtra()->bhidecrosshair = true;
				else
					ZGetConfiguration()->GetExtra()->bhidecrosshair = false;
		}

		MButton* pbullet = (MButton*)pResource->FindWidget("bullet");
		if(pbullet)
		{
			Z_EXTRA_BULLET = pbullet->GetCheck();

				if (Z_EXTRA_BULLET)
					ZGetConfiguration()->GetExtra()->bbullet = true;
				else
					ZGetConfiguration()->GetExtra()->bbullet = false;
		}

				MButton* psword = (MButton*)pResource->FindWidget("sword");
		if(psword)
		{
			Z_EXTRA_SWORD = psword->GetCheck();

				if (Z_EXTRA_SWORD)
					ZGetConfiguration()->GetExtra()->bsword = true;
				else
					ZGetConfiguration()->GetExtra()->bsword = false;
		}

		MButton* phpap = (MButton*)pResource->FindWidget("hpap");
		if(phpap)
		{
			Z_EXTRA_HPAP = phpap->GetCheck();

				if (Z_EXTRA_HPAP)
					ZGetConfiguration()->GetExtra()->bhpap = true;
				else
					ZGetConfiguration()->GetExtra()->bhpap = false;
		}
		MButton* pvisible = (MButton*)pResource->FindWidget("visible");
		if(pvisible)
		{
			Z_EXTRA_VISIBLE = pvisible->GetCheck();

				if (Z_EXTRA_VISIBLE)
					ZGetConfiguration()->GetExtra()->bvisible = true;
				else
					ZGetConfiguration()->GetExtra()->bvisible = false;
		}

		MButton* pwheellock = (MButton*)pResource->FindWidget("wheellock");
		if(pwheellock)
		{
			Z_EXTRA_WHEELLOCK = pwheellock->GetCheck();

				if (Z_EXTRA_WHEELLOCK)
					ZGetConfiguration()->GetExtra()->bwheellock = true;
				else
					ZGetConfiguration()->GetExtra()->bwheellock = false;
		}

		MButton* pnonumbers = (MButton*)pResource->FindWidget("nonumbers");
		if(pnonumbers)
		{
			Z_EXTRA_NONUMBERS = pnonumbers->GetCheck();

				if (Z_EXTRA_NONUMBERS)
					ZGetConfiguration()->GetExtra()->bnonumbers = true;
				else
					ZGetConfiguration()->GetExtra()->bnonumbers = false;
		}

		MButton* pscops = (MButton*)pResource->FindWidget("scops");
		if(pscops)
		{
			Z_EXTRA_SCOPS = pscops->GetCheck();

				if (Z_EXTRA_SCOPS)
					ZGetConfiguration()->GetExtra()->bscops = true;
				else
					ZGetConfiguration()->GetExtra()->bscops = false;
		}

		MButton* pnosmoke = (MButton*)pResource->FindWidget("nosmoke");
		if(pnosmoke)
		{
			Z_EXTRA_NOSMOKE = pnosmoke->GetCheck();

				if (Z_EXTRA_NOSMOKE)
					ZGetConfiguration()->GetExtra()->bnosmoke = true;
				else
					ZGetConfiguration()->GetExtra()->bnosmoke = false;
		}
		...
		if(pdamage)
		{
			Z_EXTRA_DAMAGE = pdamage->GetCheck();

				if (Z_EXTRA_DAMAGE)
					ZGetConfiguration()->GetExtra()->bdamage = true;
				else
					ZGetConfiguration()->GetExtra()->bdamage = false;
		}

	    MButton* ptime = (MButton*)pResource->FindWidget("time");
		if(ptime)
		{
			Z_EXTRA_TIME = ptime->GetCheck();

				if (Z_EXTRA_TIME)
					ZGetConfiguration()->GetExtra()->btime = true;
				else
					ZGetConfiguration()->GetExtra()->btime = false;
		}

		MButton* pposition = (MButton*)pResource->FindWidget("position");
		if(pposition)
		{
			Z_EXTRA_POSITION = pposition->GetCheck();

				if (Z_EXTRA_POSITION)
					ZGetConfiguration()->GetExtra()->bposition = true;
				else
					ZGetConfiguration()->GetExtra()->bposition = false;
		}
		MButton* pautorec = (MButton*)pResource->FindWidget("autorec");
		if(pautorec)
		{
			Z_EXTRA_AUTOREC = pposition->GetCheck();

				if (Z_EXTRA_AUTOREC)
					ZGetConfiguration()->GetExtra()->bautorec = true;
				else
					ZGetConfiguration()->GetExtra()->bautorec = false;
		}
		MComboBox* pComboBox = (MComboBox*)pResource->FindWidget("CrossHairComboBox");
		if (pComboBox)
		{
