	...
	case MWT_SNIFER:
		{
			...
			ZGetConfiguration()->SetMouseSensitivityInFloat(ZGetConfiguration()->GetMouseSensitivityInFloat() / 6.0f);
		}
		break;
	}
...
void ZWeaponScreenEffect::Create()
{
	...
	if ((nPrimaryWeaponType == MWT_SNIFER) || (nSecondaryWeaponType == MWT_SNIFER))
	{
		...
		if (m_pSniferPanel) delete m_pSniferPanel;
		m_pSniferPanel = new MBitmapR2;		
		((MBitmapR2*)m_pSniferPanel)->Create("snifer.png", RGetDevice(), "interface/Default/Combat/snifer11.png");
		}
	}
}
